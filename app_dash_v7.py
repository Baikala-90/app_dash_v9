...
def cleanse_daily(df: pd.DataFrame) -> pd.DataFrame:
    ...
    col_date = next((c for c in d.columns if c in ['날짜','날짜(년월일)','일자','날']), d.columns[0])
    col_cnt   = next((c for c in d.columns if c in ['총발주종수','총발주건수','총발주건','종수']), None)
    ...
    d['__year_header__'] = d[col_date].astype(str).str.extract(r'(^\s*(\d{4})\s*년\s*$)', expand=True)[1]
    d['__year_header__'] = d['__year_header__'].ffill()

    raw = d[col_date].astype(str).str.strip().str.replace(r'\(.+?\)', '', regex=True).str.strip()
    ...
    d['날짜'] = pd.to_datetime(d['__full__'], errors='coerce', format='mixed', yearfirst=True)
    ...
    blanks = is_blank(col_cnt) & is_blank(col_total) & is_blank(col_bw) & is_blank(col_color)
    d = d.loc[~blanks].copy()

    for src, std in [(col_total, '총발주부수'), (col_bw, '흑백페이지'), (col_color, '컬러페이지')]:
        if src and src in d.columns:
            d[std] = pd.to_numeric(d[src].astype(str).str.replace(',', '', regex=False), errors='coerce').fillna(0)
        else:
            d[std] = 0
...
def figure_weekly_today_based(df_daily: pd.DataFrame) -> go.Figure:
    ...
    m = df_daily.set_index('date_only')['총발주부수'].to_dict() if not df_daily.empty else {}
    ...
    this_dates_str = [pd.Timestamp(d).strftime('%Y-%m-%d') for d in this_week_dates]
    last_dates_str = [pd.Timestamp(d).strftime('%Y-%m-%d') for d in last_week_dates]
    fig = go.Figure()
...
def figure_weekly_fixed_mon_fri(df_daily: pd.DataFrame, monday_str: str=None) -> go.Figure:
    ...
    last_dates_str = [pd.Timestamp(d).strftime('%Y-%m-%d') for d in prev_week_days]
...
def figure_months_1to12(df_monthly: pd.DataFrame, start_year=2022, current_year=None) -> go.Figure:
    ...
    d = df_monthly[(df_monthly['연도'] >= start_year) & (df_monthly['연도'] <= current_year)].copy()
    if d.empty:
        return go.Figure()
    ...
    fig = px.line(pivot, x=pivot.index, y=pivot.columns, markers=True, title=f'월별 발주량 (1~12월, {start_year}~{current_year})')
...
def compute_kpis(df_daily: pd.DataFrame, year: int):
    ...
    total_bw_pages = int(year_df['흑백페이지'].sum()) if '흑백페이지' in year_df.columns and not year_df.empty else 0
    total_color_pages = int(year_df['컬러페이지'].sum()) if '컬러페이지' in year_df.columns and not year_df.empty else 0
    return total_orders, avg_per_day, days_count, total_bw_pages, total_color_pages

def kpi_card(title, value, subtitle):
    return html.Div(style={
        'flex':'1 1 220px', 'background':'white', 'borderRadius':'14px', 'padding':'14px 16px',
        'boxShadow':'0 6px 18px rgba(0,0,0,0.08)', 'minWidth':'200px'
    }, children=[
        html.Div(title, style={'fontSize':'0.95rem','color':'#666','marginBottom':'6px','fontWeight':'600'}),
        html.Div(value, style={'fontSize':'1.6rem','fontWeight':'800'}),
        html.Div(subtitle, style={'fontSize':'0.85rem','color':'#8a8a8a','marginTop':'4px'}),
    ])

def kpi_row(total_orders, avg_per_day, days_count, total_bw_pages, total_color_pages, year):
    return html.Div(style={'display':'flex','gap':'12px','flexWrap':'wrap','marginBottom':'8px'}, children=[
        ...
        kpi_card(f"{year}년 총 발주 일수", f"{days_count:,}일", "Working Days"),
        kpi_card(f"{year}년 흑백 페이지 합계", f"{total_bw_pages:,}", "BW Pages (Daily sum)"),
        kpi_card(f"{year}년 컬러 페이지 합계", f"{total_color_pages:,}", "Color Pages (Daily sum)"),
    ])

def previous_years_table(df_daily: pd.DataFrame, current_year: int):
    if df_daily.empty: return html.Table([])
    years = sorted(df_daily['연도'].unique())
    rows = []
    for y in years:
        if y == current_year: continue
        tot, avg, days, bw, col = compute_kpis(df_daily, y)
...
CURRENT_YEAR = datetime.now(KST).year
years_options = [{'label': f'{int(y)}년', 'value': int(y)} for y in sorted(DF_DAILY['연도'].unique()) if int(y) <= CURRENT_YEAR] if not DF_DAILY.empty else []

# Charts
fig_week_today = figure_weekly_today_based(DF_DAILY) if not DF_DAILY.empty else go.Figure()
fig_months = figure_months_1to12(DF_MONTHLY, start_year=2022, current_year=CURRENT_YEAR) if not DF_MONTHLY.empty else go.Figure()
metric_figs = build_metric_figs(DF_MONTHLY, baseline_year=CURRENT_YEAR) if not DF_MONTHLY.empty else {'avg_per_day':go.Figure(),'monthly_total':go.Figure(),'bw_pages':go.Figure(),'color_pages':go.Figure()}

week_options = week_options_from_df(DF_DAILY)

BASE_STYLE = {'fontFamily': 'Noto Sans KR, Malgun Gothic, Arial'}
CARD_STYLE = {'background':'white','borderRadius':'12px','padding':'14px','boxShadow':'0 4px 14px rgba(0,0,0,0.08)'}

app.layout = html.Div(style={'maxWidth':'1100px','margin':'0 auto','padding':'16px', **BASE_STYLE}, children=[
    html.H1("발주량 분석 대시보드", style={'textAlign':'center','marginBottom':'6px'}),
    html.P("구글 시트 데이터 기반 · 2021~현재", style={'textAlign':'center','marginBottom':'14px','color':'#666'}),
    ...
    html.Div(style={'display':'flex','gap':'10px','justifyContent':'center','alignItems':'center','marginBottom':'8px'}, children=[
        ...
        dcc.Dropdown(id='year-select', options=years_options,
                     value=(years_options[-1]['value'] if years_options else CURRENT_YEAR),
    ...
    html.Div(style={'display':'grid','gridTemplateColumns':'1fr','gap':'12px','marginBottom':'12px'}, children=[
        html.Div([
            html.Div(style={'display':'flex','justifyContent':'space-between','alignItems':'center'}, children=[
                html.H3("주간 발주량 비교 (오늘 기준 5영업일 + 지난주 같은 요일)", style={'marginBottom':'4px','fontSize':'1.05rem'}),
            ]),
            dcc.Graph(id='weekly-chart-today', figure=fig_week_today, style={'height':'300px'})
        ], style=CARD_STYLE),
    ]),
    ...
    html.Details(open=False, children=[
        ...
        html.Div(style={'display':'flex','gap':'8px','alignItems':'center','margin':'8px 0'}, children=[
            ...
            dcc.Dropdown(id='week-select-fixed', options=week_options, value='this_week', clearable=False, style={'width':'300px'}),
        ]),
    ...
    html.Div(style={'textAlign':'center','color':'#888','marginTop':'16px','fontSize':'.9rem'}, children="© 2025 발주량 분석 대시보드")
])
...
def update_kpis(selected_year):
    try:
        tot, avg, days, bw, color = compute_kpis(DF_DAILY, selected_year)
        cards = kpi_row(tot, avg, days, bw, color, selected_year)
