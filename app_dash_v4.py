...
def find_credentials_path():
    for p in [os.getenv("GOOGLE_APPLICATION_CREDENTIALS"), os.getenv("GSPREAD_CREDENTIALS"), "credentials.json", "service_account.json"]:
        if p and os.path.exists(p):
            ...
            return p
    raise FileNotFoundError("서비스 계정 JSON을 찾을 수 없습니다. (credentials.json / service_account.json)")
...
def load_data_from_gsheet():
    ...
    print(f"[LOAD] daily rows: {len(df_daily_raw)}, monthly rows: {len(df_monthly_raw)}")
    return df_daily_raw, df_monthly_raw

# ---------------------------------------------
# Cleanse - Daily (연도 헤더/공휴일 제외)
# ---------------------------------------------
def cleanse_daily(df: pd.DataFrame) -> pd.DataFrame:
    ...
    col_cnt   = next((c for c in d.columns if c in ['총발주종수','총발주건수','총발주건','종수']), None)  # B
    col_total = next((c for c in d.columns if c in ['총발주부수','총발주량','총발주부','총발주수','총발주']), None)  # C
    col_bw    = next((c for c in d.columns if '흑백' in c), None)  # D
    col_color = next((c for c in d.columns if ('컬러' in c or '칼라' in c)), None)  # E

    # 연도 헤더 행 처리 (e.g., "2025년")
...
def figure_weekly(df_daily: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        x=x_week, y=y_last, mode='lines+markers', name='지난 주',
        line=dict(width=2, dash='dot'),
        customdata=last_dates_str,
        hovertemplate="%{customdata}<br>지난 주: %{y:,}부<extra></extra>"
    ))
    fig.add_trace(go.Scatter(
        x=x_week, y=y_this, mode='lines+markers', name='이번 주',
        line=dict(width=3),
...
def figure_months_1to12(df_monthly: pd.DataFrame, start_year=2022, current_year=None) -> go.Figure:
    ...
    # columns 필터링(혹시라도 이상치 연도 포함되면 제거)
...
def yoy_line_value_bar_rate(d: pd.DataFrame, value_col: str, title: str, baseline_year: int) -> go.Figure:
    if d.empty:
        return go.Figure()
    # 기준 연도(baseline_year)와 직전 연도만 YoY 막대에 사용
    d = d.copy()
    d = d[d['연도'] <= baseline_year]  # 미래 연도 제거
    ...
    fig = go.Figure()
    # 값은 모든 연도 라인 (2022~baseline_year까지)
...
app.layout = html.Div(style={'maxWidth':'1100px','margin':'0 auto','padding':'16px', **BASE_STYLE}, children=[
    ...
    html.Div(id='kpi-cards'),  # 가로 배치 KPI
...
def switch_metric_tab(tab_value):
    ...
    fig.update_layout(height=460)  # 탭 그래프 크게
