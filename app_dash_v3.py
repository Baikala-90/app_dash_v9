...
def figure_weekly(df_daily: pd.DataFrame) -> go.Figure:
    ...
    # 각 트레이스마다 실제 날짜를 hover에 보여주기 위한 customdata
...
def yoy_line_value_bar_rate(d: pd.DataFrame, value_col: str, title: str) -> go.Figure:
    if d.empty:
        return go.Figure()
    ...
    fig.add_trace(go.Bar(
        x=yoy_latest['월번호'], y=yoy_latest['YoY%'], name=f'{latest_year} YoY%',
...
def build_metric_figs(df_monthly: pd.DataFrame):
    ...
    return {
        'avg_per_day': yoy_line_value_bar_rate(d, '일평균발주량', '월별 일평균 발주량 + YoY%'),
        'monthly_total': yoy_line_value_bar_rate(d, '발주량', '월 총 발주량 + YoY%'),
        'bw_pages': yoy_line_value_bar_rate(
            d.rename(columns={next((c for c in d.columns if '흑백' in c), '흑백출력량'):'흑백출력량'}), '흑백출력량', '월별 흑백 페이지 + YoY%'
        ),
        'color_pages': yoy_line_value_bar_rate(
            d.rename(columns={next((c for c in d.columns if ('컬러' in c or '칼라' in c)), '컬러출력량'):'컬러출력량'}), '컬러출력량', '월별 컬러 페이지 + YoY%'
        ),
    }
...
years_options = [{'label': f'{int(y)}년', 'value': int(y)} for y in sorted(DF_DAILY['연도'].unique())] if not DF_DAILY.empty else []
